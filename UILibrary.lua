...
if (not isfile('Alchemia/configs')) then
...
function Signal:Connect(Callback)
    if (self.BindableEvent == nil) then
        warn("Signal no longer exists")
        return
    end
...
do -- // Load
    ...
    local function createBind(option, parent)
        ...
        interest.InputEnded:connect(function(input)
            if input.UserInputType.Name == 'MouseButton1' and not busy then
                ...
                connections.keybindLoop = RunService.Heartbeat:Connect(function()
                    ...
                    if (#displayKeys == 3) then
                        connections.keybindLoop:Disconnect()
                        connections.keybindLoop = nil;
                    end;
                end);

                task.wait(0.05);
                connections.onInputEnded = UserInputService.InputEnded:Connect(function(input)
                    if(input.UserInputType ~= Enum.UserInputType.Keyboard and not input.UserInputType.Name:find('MouseButton')) then return; end;

                    connections.keybindLoop:Disconnect()
                    connections.onInputEnded:Disconnect()
                    
                    connections.keybindLoop = nil;
                    connections.onInputEnded = nil;
            ...
            if (self.key == 'none') then
                ...
                library.OnKeyPress:Connect(function()
                    ...
                    if option.mode == 'toggle' then
                        ...
                        option.callback(option.callback, library.flags[option.flag], 0)
                    else
                        library.flags[option.flag] = true
                        ...
                        connections.Loop = library:AddConnection(RunService.Heartbeat, function(step)
    ...
    local function createList(option, parent)
        ...
        layout.Changed:Connect(function() updateHolder(); end);
        local interest = option.sub and option.listvalue or option.main
        local focused = false;

        option.listvalue.Focused:Connect(function() focused = true; end);
        option.listvalue.FocusLost:Connect(function() focused = false; end);

        option.listvalue:GetPropertyChangedSignal('Text'):Connect(function()
        ...
        function tab:AddColumn()
            ...
            function column:AddSection(title)
                ...
                function section:AddToggle(option)
                    ...
                    option.type = 'toggle'
                    option.position = #self.options
                    option.flag = (library.flagprefix or '') .. (option.flag or option.text)
                    option.subcount = 0
    ...
    function library:Init(silent)
        ...
        local searchLabel = library:Create('ImageLabel', {
            Position = UDim2.new(0, titleTextSize.X + 10, 0.5, -8.5),
        ...
        searchLabel.InputBegan:Connect(function(inputObject)
    ...
    do -- // Load Basics
        ...
        do
            ...
            settingsMain:AddList({
                ...
                values = scaleTypes,
                callback = function()
                    library.main.ScaleType = Enum.ScaleType.Tile;
                end
            });
        end;
        ...
        settingsMenu:AddColor({
            ...
            color = Color3.fromRGB(66, 133, 167),
        ...
        configSection:AddButton({
            ...
            callback = function()
                ...
                local configData = readfile(string.format('Alchemia/configs/%s/%s', folderName, fullConfigName));
